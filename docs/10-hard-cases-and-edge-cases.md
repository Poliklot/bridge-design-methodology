...
| Mobile-only copy replaces the real element | States, actions, analytics, and content can drift between copies. | Use one identity; change visibility/order/layout instead of duplicating. | Duplicate role/content heuristic. |
...
## 3. Layout and geometry nightmares

| Case | Why it is dangerous | BRIDGE response | Auto-check |
| --- | --- | --- | --- |
| Related items are free-floating siblings | Responsive transfer cannot infer grouping. | Use flow containers with explicit roles. | Geometry clustering heuristic. |
...
| Absolute element behaves like layout content | It will not reflow. | Move to flow or declare absolute reason. | Absolute-without-tag check. |
| Overlap is accidental | z-order differs across targets. | Declare `[overlay]`, `[abs]`, or collision reason. | Bounding-box overlap heuristic. |
