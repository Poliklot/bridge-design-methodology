...
| --- | --- | --- | --- |
| Один логический элемент имеет разные keys на адаптивах | Адаптер создаст дубликаты вместо одного responsive-элемента. | Один стабильный `[key=...]`. | Сравнение breakpoint groups. |
| Один key указывает на разные типы элементов | Кнопка становится текстом, картинка — карточкой и т.д. | Key должен сохранять logical type, если нет declared variant. | Сравнение type по key. |
| Повторяющиеся карточки используют один key | Нельзя отличить instances. | Индексированные keys: `card-1`, `card-2`. | Duplicate key check. |
...
| Ситуация | Почему опасно | Ответ BRIDGE | Автопроверка |
| --- | --- | --- | --- |
| Mobile copy меняет продуктовый смысл | Пользователь видит разные обещания на разных устройствах. | Same key = same meaning; для отличий нужен `[content-variant=...]`. | Text diff по key. |
| Критичный элемент исчезает на mobile | CTA, legal note, price или error state потерян. | Явно помечать intentional exclusions или сохранять key на обязательных breakpoint’ах. | Key coverage check. |
...
| Modal target отсутствует | Нельзя построить flow. | `action=modal:x` требует `[modal] [key=x]`. | Interaction graph check. |
...
| Dark mode частичный | Элементы пропадают или конфликтуют. | Theme coverage. | Theme key coverage. |
